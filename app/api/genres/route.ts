...
export async function GET(request: NextRequest) {
  ...
  try {
    // Search for comics by genre using Comic Vine search
    const response = await fetch(
      `https://comicvine.gamespot.com/api/search/?api_key=${apiKey}&format=json&query=${encodeURIComponent(genre)}&resources=volume&limit=${limit}&offset=${offset}&field_list=id,name,image,description,site_detail_url,date_added,deck,publisher,count_of_issues,start_year`,
      {
        headers: {
    ...
    return NextResponse.json({ error: "Failed to fetch comics by genre" }, { status: 500 })
  }
}
